	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if(uDAO.checkOldPassword(userId, oldPassword)) {
			...
			if(uDAO.changePassword(userId, newPassword)) {
				...
				session.setAttribute("errorMsg", "Something wrong on server!"); // Stored in Amazon ElastiCache for Redis
				resp.sendRedirect("change_password.jsp");
				
			}
			
		}else {
			session.setAttribute("errorMsg", "Old password incorrect"); // Stored in Amazon ElastiCache for Redis
			resp.sendRedirect("change_password.jsp");
		}
